...
const PlusIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>;
...
export default function PoeshApp() {
    ...
    return (
        <div className="bg-[#0a0a0a] text-white min-h-screen font-sans selection:bg-[#ff5722] selection:text-white pb-20 overflow-x-hidden">
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-8 h-20 flex justify-between items-center">
                    <div className="flex items-center gap-4 md:gap-8">
                        ...
                        <button
                            ...
                            className="hidden md:flex items-center gap-2 bg-white/5 hover:bg-white/10 px-4 py-2.5 rounded-full text-sm font-medium text-gray-300 transition-colors shrink-0"
                        >
                            <MapPinIcon />
                    ...
                    <div className="flex items-center gap-3 md:gap-5">
                        <button
                            ...
                            className="hidden md:flex items-center gap-2 text-sm font-bold bg-white/5 hover:bg-white/10 px-5 py-2.5 rounded-full transition-colors shrink-0"
                        >
                            <UserIcon />
                            Войти
                        </button>
                        ...
                        <button
                            ...
                            className="relative flex items-center justify-center gap-2 bg-[#ff5722] hover:bg-[#ff8a50] text-white px-5 py-2.5 md:px-6 md:py-3 rounded-full transition-colors shadow-[0_0_15px_rgba(255,87,34,0.25)] shrink-0"
                        >
                            <CartIcon />
                            <span className="font-bold text-sm md:text-base hidden sm:inline">{totalPrice > 0 ? `${totalPrice} ₽` : "Корзина"}</span>
                            {totalItems > 0 && (
                                <motion.span
                                    initial={{ scale: 0 }} animate={{ scale: 1 }}
                                    className="absolute -top-1.5 -right-1.5 bg-white text-black text-[11px] font-black w-5 h-5 flex items-center justify-center rounded-full sm:hidden"
                                >
                                    {totalItems}
                                </motion.span>
                            )}
            ...
            <section className="pt-28 md:pt-32 pb-8 px-4 md:px-8 max-w-7xl mx-auto">
                <div className="bg-[#ff5722] rounded-3xl h-[340px] md:h-[420px] flex items-center justify-between overflow-hidden relative">
                    <div className="relative z-10 w-full md:w-1/2 p-8 md:p-12 lg:p-16 flex flex-col justify-center items-start h-full">
                        <button onClick={() => setIsMapOpen(true)} className="md:hidden flex items-center gap-2 bg-black/20 backdrop-blur-md px-4 py-2 rounded-full text-[11px] sm:text-xs font-medium text-white mb-6 w-max shrink-0">
                            <MapPinIcon /> <span className="truncate max-w-[180px]">{address}</span>
                        </button>
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black uppercase leading-[1.1] mb-4 md:mb-6 text-black tracking-tight">
                            Быстрая<br />доставка<br />в Оренбурге
                        </h2>
                        <p className="text-black/80 font-medium text-sm md:text-base lg:text-lg mb-8 max-w-md hidden sm:block leading-snug">
                            Горячие бургеры, свежая пицца и роллы. Привезем за 45 минут или отдадим заказ бесплатно.
                        </p>
                        <button
                            onClick={() => document.getElementById('menu')?.scrollIntoView({ behavior: 'smooth' })}
                            className="bg-black text-white px-8 md:px-10 py-3.5 md:py-4 rounded-full font-bold uppercase tracking-widest text-xs md:text-sm hover:scale-105 transition-transform w-max mt-auto sm:mt-0 shrink-0"
                        >
                            Смотреть меню
                        </button>
                    </div>
                    ...
                    <div className="absolute right-0 top-0 bottom-0 w-1/2 hidden md:block">
                        <div className="absolute inset-0 bg-gradient-to-r from-[#ff5722] via-[#ff5722]/80 to-transparent z-10 w-1/3"></div>
                        <img
            ...
            <main id="menu" className="px-4 md:px-8 max-w-7xl mx-auto">
                <div className="flex gap-2 md:gap-3 overflow-x-auto pb-4 md:pb-6 scrollbar-hide items-center border-b border-white/10 mb-6 md:mb-8 sticky top-20 z-30 bg-[#0a0a0a]/95 backdrop-blur-md pt-4">
                    {categories.map(cat => (
                        <button
                            key={cat}
                            onClick={() => setActiveCategory(cat)}
                            className={`whitespace-nowrap px-6 py-2.5 rounded-full font-bold text-xs md:text-sm transition-colors shrink-0 ${activeCategory === cat
                                ? "bg-white text-black"
                ...
                <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    <AnimatePresence mode="popLayout">
                        {filteredMenu.map(item => (
                            ...
                            >
                                <div className="h-44 md:h-52 w-full overflow-hidden relative shrink-0">
                                    ...
                                    <span className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full text-[10px] uppercase font-bold tracking-widest text-white">
                                        {item.category}
                                    </span>
                                </div>
                                ...
                                <div className="p-5 md:p-6 flex flex-col flex-1 justify-between">
                                    <div>
                                        <h3 className="text-lg md:text-xl font-bold mb-2 leading-tight">{item.name}</h3>
                                    ...
                                    <div className="flex justify-between items-center mt-auto">
                                        ...
                                        <div className="relative flex items-center justify-center shrink-0">
                                            <motion.button
                                                ...
                                                className="bg-white/10 hover:bg-[#ff5722] text-white w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-colors shadow-sm shrink-0"
                                            >
                                                <PlusIcon />
                                            </motion.button>
                ...
                {isMapOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-[#111] border border-white/10 rounded-2xl md:rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl relative flex flex-col max-h-[90vh]">
                            ...
                            <div className="w-full h-[40vh] md:h-72 bg-gray-800 relative shrink-0">
                            ...
                            <div className="p-5 md:p-8 bg-[#111] shrink-0">
                                <label className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-3 block">Введите улицу и дом</label>
                                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                                    <input
                                        ...
                                        className="flex-1 bg-white/5 border border-white/10 rounded-xl px-5 py-3.5 md:py-4 text-sm md:text-base text-white focus:outline-none focus:border-[#ff5722] transition-colors"
                                    />
                                    <button onClick={() => setIsMapOpen(false)} className="bg-[#ff5722] text-white px-8 py-3.5 md:py-4 rounded-xl font-bold uppercase tracking-widest text-xs md:text-sm hover:bg-[#ff8a50] transition-colors whitespace-nowrap shrink-0">
                                        Сохранить
                                    </button>
                                </div>
                            </div>
                ...
                {isAuthOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-[#111] border border-white/10 rounded-3xl w-full max-w-md p-8 relative shadow-2xl">
                            <button onClick={() => setIsAuthOpen(false)} className="absolute top-5 right-5 text-gray-500 hover:text-white p-2"><CloseIcon /></button>
                            ...
                            <div className="flex flex-col gap-4">
                                <div>
                                    ...
                                    <input type="tel" placeholder="+7 (999) 000-00-00" className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-sm md:text-base text-white focus:outline-none focus:border-[#ff5722] transition-colors" />
                                </div>
                                <button onClick={() => setIsAuthOpen(false)} className="w-full bg-[#ff5722] text-white px-6 py-4 rounded-xl font-bold uppercase tracking-widest text-xs md:text-sm hover:bg-[#ff8a50] transition-colors mt-2 shrink-0">
                                    Получить код
                                </button>
                            </div>
                ...
                {isCartOpen && (
                    <>
                        ...
                        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed top-0 right-0 w-full sm:w-[400px] md:w-[450px] h-full bg-[#111] border-l border-white/10 z-[100] flex flex-col shadow-2xl">
                            ...
                            <div className="p-5 md:p-6 border-b border-white/10 flex justify-between items-center bg-[#0a0a0a] shrink-0">
                                    ...
                                    cart.map(item => (
                                        <motion.div layout key={item.id} className="flex gap-4 items-center bg-white/5 p-4 rounded-xl border border-white/5">
                                            ...
                                            <div className="flex items-center bg-black rounded-full overflow-hidden border border-white/10 shrink-0">
                                                <button onClick={() => updateQty(item.id, -1)} className="w-10 h-10 flex items-center justify-center hover:bg-white/10 text-gray-300">-</button>
                            ...
                            {cart.length > 0 && (
                                <div className="p-5 md:p-6 bg-[#0a0a0a] border-t border-white/10 shrink-0">
                                    ...
                                    <button onClick={handleCheckout} disabled={orderPlaced} className={`w-full py-4 rounded-xl font-bold uppercase tracking-widest text-xs md:text-sm transition-all shrink-0 ${orderPlaced ? "bg-green-500 text-white" : "bg-[#ff5722] hover:bg-[#ff8a50] text-white shadow-[0_0_20px_rgba(255,87,34,0.3)]"}`}>
