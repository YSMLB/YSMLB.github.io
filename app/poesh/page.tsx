...
export default function PoeshApp() {
    ...
    const [cart, setCart] = useState<{ id: number; qty: number;[key: string]: any }[]>([]);
    ...
    // Memoize menu list to avoid unnecessary recalculations on re-renders
    ...
    return (
        <div className="bg-[#0a0a0a] text-white min-h-screen font-sans selection:bg-[#ff5722] selection:text-white pb-20 overflow-x-hidden">
            <header className="fixed top-0 left-0 w-full z-40 bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/10">
                <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 md:h-20 flex justify-between items-center">
                    <div className="flex items-center gap-4 md:gap-8">
                        ...
                        <h1 className="text-xl md:text-3xl font-black tracking-tighter uppercase text-[#ff5722]">
                            ПОЕШЬ
                        </h1>
                        <button
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        >
                            <CartIcon />
                            <span className="font-bold text-sm hidden md:inline">{totalPrice > 0 ? `${totalPrice} ₽` : "Корзина"}</span>
                            {totalItems > 0 && (
                                <motion.span
                                    initial={{ scale: 0 }} animate={{ scale: 1 }}
                                    className="absolute -top-1 -right-1 bg-white text-black text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full md:hidden"
                                >
                                    {totalItems}
                                </motion.span>
                            )}
            ...
            <section className="pt-24 md:pt-28 pb-8 px-4 md:px-8 max-w-7xl mx-auto">
                <div className="bg-gradient-to-r from-[#ff5722] to-[#ff8a50] rounded-3xl h-[320px] md:h-[400px] flex items-center justify-between overflow-hidden relative shadow-2xl">
                    <div className="relative z-10 w-full md:w-1/2 p-6 md:p-16 flex flex-col justify-center h-full">
                        <button onClick={() => setIsMapOpen(true)} className="md:hidden flex items-center gap-2 bg-black/20 backdrop-blur-md px-4 py-2 rounded-full text-[10px] sm:text-xs font-medium text-white mb-4 w-max">
                            <MapPinIcon /> <span className="truncate max-w-[150px]">{address}</span>
                        </button>
                        <h2 className="text-3xl sm:text-4xl md:text-6xl font-black uppercase leading-tight mb-3 md:mb-4 text-black">
                            Быстрая доставка <br className="hidden md:block" />в Оренбурге
                        </h2>
                        <p className="text-black/80 font-medium text-sm md:text-lg mb-6 md:mb-8 max-w-md hidden sm:block">
                            Горячие бургеры, свежая пицца и роллы. Привезем за 45 минут или отдадим заказ бесплатно.
                        </p>
                        <button
                            onClick={() => document.getElementById('menu')?.scrollIntoView({ behavior: 'smooth' })}
                            className="bg-black text-white px-6 md:px-8 py-3 md:py-4 rounded-full font-bold uppercase tracking-widest text-xs md:text-sm hover:scale-105 transition-transform w-max mt-auto sm:mt-0"
                        >
                            Смотреть меню
                        </button>
                    </div>
            ...
            <main id="menu" className="px-4 md:px-8 max-w-7xl mx-auto">
                <div className="flex gap-2 md:gap-4 overflow-x-auto pb-4 md:pb-6 scrollbar-hide items-center border-b border-white/10 mb-6 md:mb-8 sticky top-16 md:top-20 z-30 bg-[#0a0a0a]/95 backdrop-blur-md pt-4">
                    {categories.map(cat => (
                        <button
                            key={cat}
                            onClick={() => setActiveCategory(cat)}
                            className={`whitespace-nowrap px-5 md:px-6 py-2 rounded-full font-bold text-xs md:text-sm transition-colors ${activeCategory === cat
                                ? "bg-white text-black"
                    ...
                    <AnimatePresence mode="popLayout">
                        {filteredMenu.map(item => (
                            <motion.div
                                ...
                                key={item.id}
                                className="bg-[#141414] rounded-2xl md:rounded-3xl overflow-hidden border border-white/5 hover:border-white/20 transition-colors flex flex-col group relative"
                            >
                                <div className="h-40 md:h-48 w-full overflow-hidden relative">
                                    ...
                                    <span className="absolute top-3 left-3 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-[9px] md:text-[10px] uppercase font-bold tracking-wider text-white">
                                        {item.category}
                                    </span>
                                </div>
                                ...
                                <div className="p-4 md:p-5 flex flex-col flex-1 justify-between">
                                    <div>
                                        <h3 className="text-lg md:text-xl font-bold mb-1 md:mb-2">{item.name}</h3>
                                        <p className="text-gray-500 text-xs md:text-sm mb-4 md:mb-6 leading-relaxed line-clamp-2">{item.desc}</p>
                                    </div>
                                    <div className="flex justify-between items-center mt-auto">
                                        <span className="text-lg md:text-xl font-black">{item.price} ₽</span>

                                        <div className="relative">
                                            <motion.button
                                                whileTap={{ scale: 0.85 }}
                                                onClick={() => addToCart(item)}
                                                className="bg-white/10 hover:bg-[#ff5722] text-white w-12 h-12 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors shadow-sm"
                                            >
                                                <PlusIcon />
                                            </motion.button>
            ...
            <AnimatePresence>
                {isMapOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-[#111] border border-white/10 rounded-2xl md:rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl relative flex flex-col max-h-[90vh]">
                            <div className="p-4 md:p-6 border-b border-white/10 flex justify-between items-center bg-[#1a1a1a] shrink-0">
                                <h3 className="text-lg md:text-xl font-bold">Куда доставить?</h3>
                                <button onClick={() => setIsMapOpen(false)} className="text-gray-400 hover:text-white p-2 -mr-2"><CloseIcon /></button>
                            </div>

                            <div className="w-full h-[40vh] md:h-64 bg-gray-800 relative shrink-0">
                            ...
                            <div className="p-4 md:p-6 bg-[#111] shrink-0">
                                <label className="text-[10px] md:text-sm text-gray-400 font-bold uppercase tracking-widest mb-2 block">Введите улицу и дом</label>
                                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                                    <input
                                        ...
                                        className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 md:py-4 text-sm text-white focus:outline-none focus:border-[#ff5722] transition-colors"
                                    />
                                    <button onClick={() => setIsMapOpen(false)} className="bg-[#ff5722] text-white px-6 py-3 md:py-4 rounded-xl font-bold uppercase tracking-widest text-xs md:text-sm hover:bg-[#ff8a50] transition-colors whitespace-nowrap">
                                        Сохранить
                                    </button>
                                </div>
                            </div>
            ...
            <AnimatePresence>
                {isAuthOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-[#111] border border-white/10 rounded-2xl md:rounded-3xl w-full max-w-md p-6 md:p-8 relative shadow-2xl">
                            <button onClick={() => setIsAuthOpen(false)} className="absolute top-4 md:top-6 right-4 md:right-6 text-gray-500 hover:text-white p-2 -mr-2 -mt-2"><CloseIcon /></button>
                            <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter mb-2">Вход в систему</h2>
                            <p className="text-gray-400 text-xs md:text-sm mb-6 md:mb-8">Чтобы копить баллы и отслеживать заказы.</p>

                            <div className="flex flex-col gap-3 md:gap-4">
                                <div>
                                    <label className="text-[10px] md:text-xs text-gray-500 font-bold uppercase tracking-widest mb-1 md:mb-2 block">Номер телефона</label>
                                    <input type="tel" placeholder="+7 (999) 000-00-00" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 md:py-4 text-sm text-white focus:outline-none focus:border-[#ff5722] transition-colors" />
                                </div>
                                <button onClick={() => setIsAuthOpen(false)} className="w-full bg-[#ff5722] text-white px-6 py-3 md:py-4 rounded-xl font-bold uppercase tracking-widest text-xs md:text-sm hover:bg-[#ff8a50] transition-colors mt-2 md:mt-4">
                                    Получить код
                                </button>
                            </div>
                            <p className="text-[10px] md:text-xs text-gray-600 text-center mt-4 md:mt-6">Продолжая, вы соглашаетесь с политикой конфиденциальности.</p>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {isCartOpen && (
                    <>
                        ...
                        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed top-0 right-0 w-full sm:w-[400px] md:w-[450px] h-full bg-[#111] border-l border-white/10 z-[100] flex flex-col shadow-2xl">

                            <div className="p-4 md:p-6 border-b border-white/10 flex justify-between items-center bg-[#0a0a0a]">
                                <h2 className="text-xl md:text-2xl font-black uppercase">Ваш заказ</h2>
                                <button onClick={() => setIsCartOpen(false)} className="text-gray-500 hover:text-white transition-colors p-2 -mr-2"><CloseIcon /></button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-3 md:gap-4">
                                {cart.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-full text-gray-500 opacity-50">
                                        <CartIcon />
                                        <p className="mt-4 font-medium text-base md:text-lg">Корзина пуста</p>
                                        <p className="text-xs md:text-sm text-center mt-2">Добавьте что-нибудь вкусное из меню</p>
                                    </div>
                                ) : (
                                    cart.map(item => (
                                        <motion.div layout key={item.id} className="flex gap-3 md:gap-4 items-center bg-white/5 p-3 md:p-4 rounded-xl border border-white/5">
                                            <img src={item.img} alt={item.name} className="w-14 h-14 md:w-16 md:h-16 rounded-lg object-cover shrink-0" />
                                            <div className="flex-1 min-w-0">
                                                <h4 className="font-bold text-xs md:text-sm leading-tight truncate">{item.name}</h4>
                                                <p className="text-[#ff5722] font-bold text-xs md:text-sm mt-1">{item.price * item.qty} ₽</p>
                                            </div>
                                            <div className="flex items-center bg-black rounded-full overflow-hidden border border-white/10 shrink-0">
                                                <button onClick={() => updateQty(item.id, -1)} className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center hover:bg-white/10 text-gray-300">-</button>
                                                <span className="w-4 md:w-6 text-center text-xs font-bold">{item.qty}</span>
                                                <button onClick={() => updateQty(item.id, 1)} className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center hover:bg-white/10 text-gray-300">+</button>
                                            </div>
                                        </motion.div>
                                    ))
                            ...
                            {cart.length > 0 && (
                                <div className="p-4 md:p-6 bg-[#0a0a0a] border-t border-white/10">
                                    <div className="flex justify-between items-center mb-4 md:mb-6">
                                        <span className="text-gray-400 font-medium text-sm md:text-base">Итого к оплате:</span>
                                        <span className="text-2xl md:text-3xl font-black">{totalPrice} ₽</span>
                                    </div>
                                    <button onClick={handleCheckout} disabled={orderPlaced} className={`w-full py-3 md:py-4 rounded-xl font-bold uppercase tracking-widest text-xs md:text-sm transition-all ${orderPlaced ? "bg-green-500 text-white" : "bg-[#ff5722] hover:bg-[#ff8a50] text-white shadow-lg shadow-[#ff5722]/20"}`}>
